  ...
  return (
    <>
      ...
      <div className="bg-white border-b border-gray-200 sticky top-[57px] z-40">
        <div className="max-w-[760px] mx-auto flex">
          {tabs.map(({ id, icon, label }) => (
            <button
              key={id}
              onClick={() => setTab(id)}
              className={`flex-1 py-3 px-2 border-none bg-transparent text-xs font-semibold flex items-center justify-center gap-1.5 border-b-2 transition-all cursor-pointer ${
                tab === id
